...
public class PatchManagementIT extends PatchTestSupport {
    ...
    public void fetchPatch1FromZipFileWithDescriptor() throws IOException {
        ...
        assertThat(data.getBundles().iterator().next(), equalTo("mvn:org.jboss.fuse/fuse-tranquility/1.2.3"));
        assertThat(data.getFiles().size(), equalTo(2));

        assertTrue(FileUtils.readFileToString(new File(karafHome, "patches/my-patch-1/bin/start"), "UTF-8").contains("echo \"started\"\n"));
        assertThat("Fetching a patch should extract maven artifacts immediately",
                FileUtils.readFileToString(new File(karafHome, "system/org/jboss/fuse/fuse-tranquility/1.2.3/fuse-tranquility-1.2.3.jar"), "UTF-8"),
                equalTo("JAR\n"));
    }
    ...
    public void fetchPatch3FromZipFileWithoutDescriptor() throws IOException {
        ...
        assertThat(data.getBundles().size(), equalTo(1));
        assertThat(data.getBundles().iterator().next(), equalTo("mvn:org.jboss.fuse/fuse-colours/1.2.3"));
        assertThat(data.getFiles().size(), equalTo(2));

        assertTrue(FileUtils.readFileToString(new File(karafHome, "patches/patch-3/bin/stop"), "UTF-8").contains("echo \"stopped\"\n"));
        assertThat("Fetching a patch should extract maven artifacts immediately",
                FileUtils.readFileToString(new File(karafHome, "system/org/jboss/fuse/fuse-colours/1.2.3/fuse-colours-1.2.3.jar"), "UTF-8"),
                equalTo("JAR\n"));
