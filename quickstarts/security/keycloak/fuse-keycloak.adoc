...
The below sections refer to single Keycloak _realm_ created in Keycloak server 4.1.0.Final for the purpose
...
* `etc/fuse7karaf-realm-export.json` - export of newly `fuse7karaf` realm after all the changes I made for the examples.
...
[listing,options="nowrap"]
----
karaf@root()> feature:repo-add mvn:org.keycloak/keycloak-osgi-features/4.1.0.Final/xml/features
Adding feature url mvn:org.keycloak/keycloak-osgi-features/4.1.0.Final/xml/features

karaf@root()> feature:install -v keycloak-jaas
Adding features: keycloak-jaas/[4.1.0.Final,4.1.0.Final]
...
...
By definition (in blueprint.xml inside `mvn:org.keycloak/keycloak-osgi-jaas/4.1.0.Final` bundle),
...
Adding features: keycloak-pax-http-undertow/[4.1.0.Final,4.1.0.Final]
...
----
...
...
359 │ Resolved │  80 │ 4.1.0.Final                 │ Keycloak Fuse 7.0 Adapter - Undertow, Hosts: 219
----

Now, pax-web-undertow will look for `io.undertow.servlet.ServletExtension` which is now exposed by
`mvn:org.keycloak/keycloak-pax-web-undertow/4.1.0.Final` fragment bundle.
...
[listing,options="nowrap"]
----
2018-06-26 18:06:01,880 INFO  {fileinstall-/data/servers/fuse-karaf-7.0.0.fuse-000191-redhat-1/etc} (PaxLoggingServiceImpl.java:398) : Creating configuration from org.ops4j.pax.web.context-cxf.cfg
...
[listing,options="nowrap"]
----
karaf@root()> install -s mvn:org.jboss.fuse.quickstarts.security/keycloak-cxf/7.1.0.redhat-SNAPSHOT
Bundle ID: 235
----
